...
public class HRegionServer extends Thread
  ...
  private ReportRegionStateTransitionRequest
    ...
    // Add exception message for FAILED_OPEN transitions
    if (code == TransitionCode.FAILED_OPEN && exception != null) {
      // Use exception's toString() which includes class name and message
      String exceptionMsg = exception.toString();
      if (exceptionMsg != null && exceptionMsg.length() > 0) {
        transition.setExceptionMessage(exceptionMsg);
      }
    }
